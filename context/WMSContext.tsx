...
interface WMSContextType {
    ...
    updateMissionStatus: (missionId: string, status: Missao['status'], operadorId?: string) => void;
    ...
    getActiveConferencia: (conferenteId: string) => { conferencia: Conferencia, pedido: Pedido, items: ConferenciaItem[] } | null;
    recordConferenciaItem: (data: Omit<ConferenciaItem, 'id'>) => { success: boolean, message?: string };
    // FIX: Changed the type of `data` to exclude `createdAt` as it's generated by the function.
    reportConferenciaError: (data: Omit<ConferenciaErro, 'id' | 'createdAt'>) => void;
    finishConferencia: (conferenciaId: string) => void;
...
export const WMSProvider: React.FC<{ children: React.ReactNode }> = ({ children }) => {
    ...
    const apontarEtiqueta = (id: string, data: Partial<Etiqueta>): { success: boolean, message?: string, warnings?: string[] } => {
        ...
        if (data.validade && sku.tempoVida) {
            ...
            if (diasRestantes < (sku.tempoVida * 0.5)) { // Alerta se tiver menos de 50% de shelf life
        ...
        for (const item of pedido.items) {
            ...
            for (const etiqueta of etiquetasDisponiveis) {
                ...
                if (quantidadeRetirar > 0) {
                    // Critical Rule: If origin is storage, it's a replenishment mission.
    ...
    const updateMissionStatus = (missionId: string, status: Missao['status'], operadorId?: string) => {
        setMissoes(prevMissoes => {
            ...
            const completedMission = updatedMission;

            if (status === 'Concluída' && completedMission?.pedidoId) {
                const pedidoId = completedMission.pedidoId;
                const allMissionsForPedido = updatedMissoes.filter(m => m.pedidoId === pedidoId && m.tipo === MissaoTipo.PICKING);
    ...
    const assignFamilyMissionsToOperator = (pedidoId: string, familia: string, operadorId: string): { success: boolean; missions?: Missao[]; message?: string } => {
        ...
        setMissoes(prevMissoes => {
            const missionsToAssign = prevMissoes.filter(m =>
                m.pedidoId === pedidoId &&
                m.status === 'Pendente' &&
                (m.tipo === MissaoTipo.PICKING || m.tipo === MissaoTipo.MOVIMENTACAO_PALLET) &&
    ...
    const startConferencia = (pedidoId: string, conferenteId: string): Conferencia | null => {
        let newConferencia: Conferencia | null = null;
        setPedidos(prev => prev.map(p => {
            if (p.id === pedidoId && p.status === 'Separado') {
                return {...p, status: 'Em Conferência'};
            }
            return p;
        }));
        ...
        setConferencias(prev => {
            ...
            return [...prev, newConferencia];
        });
        return newConferencia;
    };

    const getActiveConferencia = (conferenteId: string): { conferencia: Conferencia, pedido: Pedido, items: ConferenciaItem[] } | null => {
        ...
        const items = conferenciaItems.filter(i => i.conferenciaId === conf.id);
        return { conferencia: conf, pedido, items };
    };

    const recordConferenciaItem = (data: Omit<ConferenciaItem, 'id'>) => {
        const newItem: ConferenciaItem = { ...data, id: generateId() };
        setConferenciaItems(prev => [...prev, newItem]);
        return { success: true };
    };

    const reportConferenciaError = (data: Omit<ConferenciaErro, 'id' | 'createdAt'>) => {
        const newError: ConferenciaErro = { ...data, id: generateId(), createdAt: new Date().toISOString() };
        setConferenciaErros(prev => [...prev, newError]);
    };

    const finishConferencia = (conferenciaId: string) => {
        const conferencia = conferencias.find(c => c.id === conferenciaId);
        const pedido = pedidos.find(p => p.id === conferencia?.pedidoId);

        if (!conferencia || !pedido) return;
        
        // Auto-generate 'Falta' errors
        const countedSkus = conferenciaItems
            .filter(i => i.conferenciaId === conferenciaId)
            .reduce((acc, item) => {
                const key = `${item.skuId}-${item.lote}`;
                if (!acc[key]) acc[key] = 0;
                acc[key] += item.quantidadeContada;
                return acc;
        ...
        pedido.items.forEach(pedidoItem => {
            ...
            const countedQty = countedSkus[key] || 0;
            
            if (countedQty < pedidoItem.quantidadeCaixas) {
                const newError: ConferenciaErro = {
                    id: generateId(),
                    conferenciaId,
                    pedidoId: pedido.id,
                    skuId: sku.id,
                    lote: pedidoItem.lote,
                    tipo: ConferenciaErroTipo.FALTA,
                    quantidadeDivergente: pedidoItem.quantidadeCaixas - countedQty,
                    observacao: 'Gerado automaticamente ao finalizar conferência.',
        ...
        setPedidos(prev => prev.map(p => p.id === conferencia.pedidoId ? { ...p, status: 'Conferido' } : p));
    };
    ...
    const value = {
        ...
        pedidos, addPedidos, updatePedido, processTransportData, generateMissionsForPedido,
        ...
        conferencias, conferenciaItems, conferenciaErros, startConferencia, getActiveConferencia, recordConferenciaItem, reportConferenciaError, finishConferencia
    };
